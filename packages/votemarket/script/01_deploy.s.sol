...
/// @dev The salt is used for deterministic deployment. It follows this template: `STAKEDAO.VOTEMARKET.V2.{gaugeOwner}.{seed}.{contractIdentifier}`
...
///
///      The salt prefix (`STAKEDAO.VOTEMARKET.V2.{gaugeOwner}.{seed}.`) is constant for all the deployments accross the chain.
...
contract VoteMarketDeploy is Script {
    ...
    function run() external postHook_updateDataBase returns (string memory saltPrefix) {
        ...
        // Generate the constant prefix that will be part of the deterministic deployment salt.
        // This prefix is the same for all the deployments across all chains.
        saltPrefix =
            string.concat("STAKEDAO.VOTEMARKET.V2.", protocol.name, ".", vm.toString(abi.encodePacked(seed)), ".");
